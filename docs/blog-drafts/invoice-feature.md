...
If your company does business across Asia, you know the pain. A stack of Korean 세금계산서 arrives from a vendor in Seoul. A Japanese supplier sends 適格請求書 in PDF. A Chinese partner emails 增值税发票 as scanned images. Now your accounting team needs to get all of that into a spreadsheet.

The options are not great:

- **Manual data entry.** Someone stares at characters they cannot read and types numbers one field at a time. At an industry average of $15 per invoice and 14.6 days of processing time, this does not scale.
- **Hire a native speaker.** Companies routinely hire bilingual staff specifically to read foreign receipts. SAP Concur's own community forums describe this as a common workaround because no automated solution handles APAC invoices well.
...
86% of small and medium businesses still enter invoice data manually. 39% of those invoices contain errors. The market for AI invoice processing is projected to reach $47 billion by 2034 — but for CJK documents specifically, there has been no good answer. Until now.
...
3. **Validation (Accountant Agent).** A second AI agent — acting as a senior CPA — reviews every extracted invoice. It cross-checks the math (do line items sum to the subtotal? does subtotal plus tax equal the total?), verifies tax rates (10% VAT for Korean invoices, 8% or 10% consumption tax for Japanese invoices), detects potential duplicates, and flags anomalies like negative totals or zero amounts.
...
The following features are on our roadmap but not yet available:

- **QuickBooks and Xero integration** — Push extracted invoices directly into your accounting software as bills.
- **Korean 세금계산서 compliance validation** — Full validation against National Tax Service requirements, including business registration number verification and required field checks.
- **Japanese 適格請求書 compliance validation** — JCT registration number verification and itemized tax rate validation.
- **Chinese 增值税发票 validation** — VAT type detection and required field checks.
- **Duplicate detection across batches** — Fuzzy matching across languages and time periods.
- **Auto-categorization** — GL code prediction based on vendor and line item descriptions.
- **Email inbox integration** — Forward invoices to a dedicated email address for automatic processing.

---

Try it now at [tryxenith.com/invoice](https://tryxenith.com/invoice).
...
해외 거래처와 일하다 보면 피할 수 없는 문제가 있습니다. 일본 거래처에서 適格請求書가 PDF로 도착하고, 중국 파트너가 增值税发票를 스캔 이미지로 보내옵니다. 반대로, 한국어 세금계산서를 해외 본사에 보고해야 하는 상황도 있습니다. 이 모든 서류를 엑셀 한 장에 정리해야 하는 건 경리팀의 몫입니다.

현실적인 선택지는 제한적입니다:

- **수작업 입력.** 읽을 수 없는 외국어를 보면서 숫자를 하나하나 옮겨 적습니다. 업계 평균 청구서 1건당 처리 비용이 $15, 처리 기간이 14.6일입니다.
- **원어민 채용.** 외국어 영수증을 읽기 위해 이중 언어 직원을 별도로 고용하는 회사가 적지 않습니다. SAP Concur 커뮤니티에서도 APAC 영수증 처리를 위한 자동화 솔루션이 없어서 원어민을 고용한다는 이야기가 나옵니다.
- **한글(HWPX)을 지원하지 않는 OCR 도구.** 대부분의 인보이스 OCR 도구는 영문 문서 기준으로 만들어져 있습니다. 한국어 세금계산서의 구조나 필수 항목을 이해하지 못합니다. HWPX 파일은 아예 열지도 못합니다.

중소기업의 86%가 여전히 청구서를 수작업으로 입력하고 있고, 입력된 청구서의 39%에 오류가 포함되어 있습니다. AI 인보이스 처리 시장은 2034년까지 470억 달러 규모로 성장할 것으로 예측되지만, CJK(한중일) 문서에 대한 제대로 된 솔루션은 없었습니다.
...
3. **검증 (Accountant Agent).** 두 번째 AI 에이전트가 공인회계사 역할로 모든 청구서를 검토합니다. 산술 검증(품목 합계가 공급가액과 일치하는지, 공급가액 + 세액이 합계와 같은지), 세율 확인(한국 세금계산서 10% VAT, 일본 適格請求書 8%/10% 소비세), 중복 감지, 이상치 플래그 처리를 수행합니다.
...
다음 기능은 개발 로드맵에 있지만 아직 제공되지 않습니다:

- **QuickBooks / Xero 연동** — 추출된 청구서를 회계 소프트웨어에 직접 전송
- **세금계산서 규정 준수 검증** — 사업자등록번호 확인, 필수 항목 검증, 국세청 기준 준수
- **適格請求書 규정 준수 검증** — JCT 등록번호 확인, 항목별 세율 검증
- **增值税发票 검증** — VAT 유형 감지 및 필수 항목 검증
- **배치 간 중복 감지** — 언어와 기간을 넘나드는 퍼지 매칭
- **자동 분류** — 거래처 및 품목 기반 GL 코드 예측
- **이메일 연동** — 전용 이메일 주소로 청구서를 전달하면 자동 처리

---

지금 바로 사용해 보세요: [tryxenith.com/invoice](https://tryxenith.com/invoice)
...
アジア圏と取引のある企業なら、この苦労を知っているはずです。韓国の取引先から세금계산서がPDFで届く。中国のパートナーが增值税发票をスキャン画像で送ってくる。あるいは、自社の適格請求書を海外本社に報告する必要がある。これらすべてを一つのスプレッドシートにまとめるのは、経理チームの仕事です。

現実的な選択肢は限られています：

- **手入力。** 読めない外国語を見ながら、一つずつ数字を打ち込む。業界平均で請求書1件あたりの処理コストは$15（約2,200円）、処理期間は14.6日です。
- **ネイティブスピーカーの雇用。** 外国語のレシートを読むためだけにバイリンガルスタッフを雇う企業は少なくありません。SAP Concurのコミュニティでも、APACの領収書処理を自動化するソリューションがないため、ネイティブスピーカーを雇っているという報告があります。
- **CJKを理解しないOCRツール。** ほとんどの請求書OCRツールは英語の文書向けに作られています。文字を読み取ることはできますが、韓国の税務請求書の構造や適格請求書の必須項目を理解していません。

中小企業の86%が未だに請求書データを手入力しており、入力された請求書の39%にエラーが含まれています。AI請求書処理市場は2034年までに470億ドル規模に成長すると予測されていますが、CJK（日中韓）文書に特化したソリューションはこれまでありませんでした。
...
3. **検証（Accountant Agent）。** 2つ目のAIエージェントが公認会計士の役割で、すべての請求書をレビューします。計算検証（明細の合計が小計と一致するか、小計+税額が合計と等しいか）、税率確認（韓国の세금계산서は10% VAT、日本の適格請求書は8%/10%消費税）、重複検知、異常値のフラグ付けを実行します。

4. **レポート生成。** 検証済みデータがExcelワークブック（またはCSV）にまとめられ、ダウンロードとして返されます。

全プロセスが数秒で完了します。数日ではありません。
...
2023年10月に開始されたインボイス制度により、適格請求書の正確な処理がこれまで以上に重要になっています。Xenithは適格請求書から以下のフィールドを自動抽出します：
...
計算検証により、税率の適用ミスや合計金額の不一致を自動的にフラグ付けします。
...
将来的には請求書単位の従量課金モデルを導入予定です。1件あたり$0.50〜1.00程度を想定しており、手作業の処理コスト$15と比較するとごくわずかです。

## 今後のリリース予定

以下の機能はロードマップに含まれていますが、まだ提供されていません：

- **QuickBooks / Xero連携** — 抽出した請求書データを会計ソフトに直接プッシュ
- **適格請求書コンプライアンス検証** — JCT登録番号の検証、必須項目チェック、項目別税率の検証
- **韓国 세금계산서 コンプライアンス検証** — 事業者登録番号の検証、必須項目チェック
- **中国 增值税发票 検証** — VAT種別の検出、必須項目チェック
- **バッチ間重複検出** — 言語と期間を横断するファジーマッチング
- **自動分類** — 取引先・明細内容に基づくGL勘定科目コードの予測
- **メール連携** — 専用メールアドレスに請求書を転送するだけで自動処理

---

今すぐお試しください：[tryxenith.com/invoice](https://tryxenith.com/invoice)
